...
const main = async () => {
  ...
  console.log(
    "Amount to burn:",
    ethers.formatUnits(burnAmount, 2),
    "PRIV tokens"
  );
...
Registering user with
 Address:${userAddress.toLowerCase()}`;
    ...
    const userKeysMatch =
      derivedUserPublicKey[0] === BigInt(userPublicKey[0].toString()) &&
    ...
    console.log(
      `User's current balance: ${ethers.formatUnits(userCurrentBalance, tokenDecimals)} PRIV`
    );

    if (userCurrentBalance < burnAmount) {
      console.error(
        `Insufficient balance to burn. Have: ${ethers.formatUnits(userCurrentBalance, tokenDecimals)}, Need: ${ethers.formatUnits(burnAmount, tokenDecimals)}`
      );
      return;
    }

    console.log(
      `Burn amount: ${ethers.formatUnits(burnAmount, tokenDecimals)} PRIV tokens`
    );
    ...
    const burnTx = await encryptedERC.privateBurn(burnProof, userBalancePCT);
    ...
    const isNewEGCTEmpty =
      newC1[0] === 0n && newC1[1] === 0n && newC2[0] === 0n && newC2[1] === 0n;
    ...
    console.log(
      `User's new balance: ${ethers.formatUnits(userNewBalance, tokenDecimals)} PRIV`
    );
    console.log(
      `Amount burned: ${ethers.formatUnits(burnAmount, tokenDecimals)} PRIV`
    );

    console.log("\nBurn Summary:");
    console.log(`   User: ${userAddress}`);
    console.log(
      `   Amount burned: ${ethers.formatUnits(burnAmount, tokenDecimals)} PRIV tokens`
    );
    console.log(
      `   Remaining balance: ${ethers.formatUnits(userNewBalance, tokenDecimals)} PRIV tokens`
    );
    ...
    console.log("\nNext Steps:");
    console.log(
      "   • Check updated balance: npx hardhat run scripts/standalone/06_check-balance.ts --network fuji"
    );
    console.log(
      "   • Mint more tokens: npx hardhat run scripts/standalone/05_mint.ts --network fuji"
    );
  } catch (error) {
    ...
    if (error instanceof Error) {
      if (error.message.includes("User not registered")) {
        ...
        console.error(
          "Hint: The burn proof verification failed - check inputs"
        );
