...
const main = async () => {
  ...
  console.log(
    "Amount to transfer:",
    ethers.formatUnits(transferAmount, 2),
    "PRIV tokens"
  );
  ...
  try {
    ...
    const isReceiverRegistered =
      await registrar.isUserRegistered(receiverAddress);

    if (!isSenderRegistered) {
      console.error(
        "‚ù?Sender is not registered. Please run the registration script first."
      );
      return;
    }
    if (!isReceiverRegistered) {
      console.error(
        "‚ù?Receiver is not registered. They need to register first."
      );
      return;
    }

    console.log("‚ú?Both sender and receiver are registered");
    ...
    if (fs.existsSync(keysPath)) {
      ...
      if (keysData.address === senderAddress) {
        ...
        console.log("‚ú?Sender keys loaded from file");
      } else {
...
Registering user with
 Address:${senderAddress.toLowerCase()}`;
    ...
    const senderKeysMatch =
      derivedSenderPublicKey[0] === BigInt(senderPublicKey[0].toString()) &&
      derivedSenderPublicKey[1] === BigInt(senderPublicKey[1].toString());

    if (!senderKeysMatch) {
      console.error(
        "‚ù?Sender's private key doesn't match registered public key!"
      );
      console.log(
    ...
    console.log("‚ú?Sender keys verified");
    ...
    if (isEGCTEmpty) {
      console.error("‚ù?Sender has no encrypted balance to transfer");
      return;
    }
    ...
    console.log(
      `üí∞ Sender's current balance: ${ethers.formatUnits(senderCurrentBalance, tokenDecimals)} PRIV`
    );

    if (senderCurrentBalance < transferAmount) {
      console.error(
        `‚ù?Insufficient balance. Have: ${ethers.formatUnits(senderCurrentBalance, tokenDecimals)}, Need: ${ethers.formatUnits(transferAmount, tokenDecimals)}`
      );
      return;
    }

    console.log(
      `‚ú?Transfer amount: ${ethers.formatUnits(transferAmount, tokenDecimals)} PRIV tokens`
    );

    // Prepare data for transfer proof generation
    const senderEncryptedBalance = [c1[0], c1[1], c2[0], c2[1]];
    const receiverPublicKeyArray = [
      BigInt(receiverPublicKey[0].toString()),
      BigInt(receiverPublicKey[1].toString()),
    ];
    const auditorPublicKeyArray = [
      BigInt(auditorPublicKey.x.toString()),
    ...
    console.log("‚ú?Transfer proof generated successfully");
    ...
    console.log(
      "‚ú?Transfer transaction confirmed in block:",
      receipt?.blockNumber
    );
    ...
    const newC1: [bigint, bigint] = [
      BigInt(newEGCT.c1.x.toString()),
      BigInt(newEGCT.c1.y.toString()),
    ];
    const newC2: [bigint, bigint] = [
      BigInt(newEGCT.c2.x.toString()),
      BigInt(newEGCT.c2.y.toString()),
    ];
    const senderNewBalance = decryptEGCTBalance(senderPrivateKey, newC1, newC2);

    console.log(
      `üí∞ Sender's new balance: ${ethers.formatUnits(senderNewBalance, tokenDecimals)} PRIV`
    );
    console.log(
      `üì§ Amount transferred: ${ethers.formatUnits(transferAmount, tokenDecimals)} PRIV`
    );
    ...
    console.error("‚ù?Error during private transfer:");
    console.error(error);
    ...
    if (error instanceof Error) {
      if (error.message.includes("User not registered")) {
        ...
        console.error(
          "üí° Hint: The auditor needs to be set in the EncryptedERC contract"
        );
      } else if (error.message.includes("InvalidProof")) {
        console.error(
          "üí° Hint: The transfer proof verification failed - check inputs"
        );
      }
    }
